...
void ofApp::setup()
{
    ...
    prevTexture.allocate(w, h, GL_RGB);
}
    ...
    if(grabber.isFrameNew())
    {
        for (int x = 0; x < w; x++)
        {
            for (int y = 0; y < h; y++)
            {
                ...
                float colorDistance = ofDistSquared(currentColor.r, currentColor.g, currentColor.b,
                                                    prevColor.r, prevColor.g, prevColor.b);
                
                if (colorDistance > threshold*threshold)
                {
        ...
        prevTexture.loadData(prevPixels);
